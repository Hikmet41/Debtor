...
var task2 = debtors.Where(debtor => debtor.Email.EndsWith("rhyta.com") || debtor.Email.EndsWith("dayrep.com"));

foreach (var debtor in task2)
{
    Console.WriteLine(debtor);
}


var task3 = debtors.Where(debtor =>
{
    int age = DateTime.Now.Year - debtor.BirthDay.Year;
    return age >= 26 && age <= 36;
});

foreach (var debtor in task3)
{
    Console.WriteLine(debtor);
}
...
var task4 = debtors.Where(debtor => debtor.Debt <= 5000);

foreach (var debtor in task4)
{
    Console.WriteLine(debtor);
}


var task5 = debtors.Where(debtor =>
    debtor.FullName.Length > 18 &&
    debtor.Phone.Count(c => c == '2' || c == '7') >= 2);

foreach (var debtor in task5)
{
    Console.WriteLine(debtor);
}


var task7 = debtors.Where(debtor => debtor.BirthDay.Month >= 12 || debtor.BirthDay.Month <= 2);

foreach (var debtor in task7)
{
    Console.WriteLine(debtor);
}
...
var task8 = debtors.Where(debtor => debtor.Debt > averageDebt)
                             .OrderBy(debtor => debtor.FullName)
                             .ThenByDescending(debtor => debtor.Debt);

foreach (var debtor in task8)
{
    Console.WriteLine(debtor);
}


var task9 = debtors.Where(debtor => !debtor.Phone.Contains("8"))
                             .Select(debtor => $"{debtor.FullName} {DateTime.Now.Year - debtor.BirthDay.Year} {debtor.Debt}");

foreach (var debtor in task9)
{
    Console.WriteLine(debtor);
}


var task11 = debtors.Where(debtor =>
{
    string fullName = debtor.FullName.ToLower();
    return fullName.GroupBy(c => c).Any(group => group.Count() >= 3);
}).OrderBy(debtor => debtor.FullName);

foreach (var debtor in task11)
{
    Console.WriteLine(debtor);
}


var task13 = debtors.GroupBy(debtor => debtor.BirthDay.Year)
                                .Select(group => new
                                {
                                    BirthYear = group.Key,
                                    MaxDebt = group.Max(debtor => debtor.Debt)
                                });

foreach (var item in task13)
{
    var debtorsWithMaxDebt = debtors.Where(debtor => debtor.BirthDay.Year == item.BirthYear && debtor.Debt == item.MaxDebt);
    foreach (var debtor in debtorsWithMaxDebt)
    {
...
var task14 = debtors.OrderByDescending(debtor => debtor.Debt).Take(5);

foreach (var debtor in task14)
{
    Console.WriteLine(debtor);
}
...
var task15 = debtors.Sum(debtor => debtor.Debt);
Console.WriteLine($"Toplam Borc: {task15}");


var task16 = debtors.Where(debtor => debtor.BirthDay.Year < 1945);

foreach (var debtor in task16)
{
    Console.WriteLine(debtor);
}
...
var task18 = debtors.Where(debtor => debtor.Phone.Distinct().Count() == debtor.Phone.Length);
var totalDebt = task18.Sum(debtor => debtor.Debt);

foreach (var debtor in task18)
{
    Console.WriteLine($"{debtor.FullName} {debtor.BirthDay.ToShortDateString()} {totalDebt}");
}



var today = DateTime.Now;
var task19 = debtors.Where(debtor =>
{
    int age = today.Year - debtor.BirthDay.Year;
    int monthsToPayOff = (debtor.Debt + 499) / 500; 
    return age >= monthsToPayOff / 12;
});

foreach (var debtor in task19)
{
    Console.WriteLine(debtor);
}


var task20 = debtors.Where(debtor =>
{
    string fullName = debtor.FullName.ToLower();
    return "smile".All(c => fullName.Contains(c));
});

foreach (var debtor in task20)
{
    Console.WriteLine(debtor);
}
